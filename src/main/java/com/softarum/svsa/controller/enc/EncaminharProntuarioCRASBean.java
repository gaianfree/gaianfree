...
//import com.softarum.svsa.util.LogUtil;
...
/**
 * Encaminhamento de famílias CREAS para o CRAS
...
@Named
@ViewScoped
public class EncaminharProntuarioCRASBean implements Serializable {

	private static final long serialVersionUID = 1769116747361287180L;
	//private LogUtil logUtil = new LogUtil(EncaminharProntuarioCRASBean.class);
	...
	private List<Unidade>  todosCRAS = new ArrayList<>();
	...
	public void inicializar() {
		...
		// recuperar todos os CRAS
		todosCRAS = unidadeService.buscarCRAS(loginBean.getTenantId());
		
		this.usuarioLogado = loginBean.getUsuario();		
		//logUtil.info("encaminharTransferenciaCRASBean criado.");
		
		/*
		if(usuarioLogado.getUnidade().getTipo() == TipoUnidade.CRAS) {
			MessageUtil.alerta("Encaminhamento para CRAS apenas CREAS pode realizar!");		
		}
		else {
			MessageUtil.alerta("Encaminhamento para CREAS apenas CRAS pode realizar!");	
		}
		*/
