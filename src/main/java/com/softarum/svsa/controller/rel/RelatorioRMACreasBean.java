...
@Named
@ViewScoped
public class RelatorioRMACreasBean implements Serializable {
	...
	private RmaCreasTO rma = new RmaCreasTO();
	...
	public void inicializar() {	
		...
		if(loginBean.getUsuario().getGrupo() == Grupo.GESTORES || loginBean.getUsuario().getUnidade().getTipo() == TipoUnidade.CRAS) {
			...
			this.rma = rmaCreasService.gerarRelatorioRMA(unidadeLogado, getAno(), getMes(), loginBean.getTenantId());
			
			rmasFechados = rmaService.buscarRmasCreasFechados(unidadeLogado, loginBean.getTenantId());
	...
	public void fecharRma() throws NegocioException {
		...
		try {
			...
			this.rma = rmaCreasService.gerarRelatorioRMA(unidadeLogado, getAno(), getMesFechar(), loginBean.getTenantId());
			
			rmaService.salvarCreas(rma, loginBean.getTenantId());
			rmasFechados = rmaService.buscarRmasCreasFechados(unidadeLogado, loginBean.getTenantId());
	...
	public void buscarRmaFechado() {
		...
		try {
			...
			this.rma = rmaService.buscarRmaCreasFechado(unidade, mesAnoRef, loginBean.getTenantId());
	...
	public void buscarRmasFechados() {
		...
		try {
			...
			rmasFechados = rmaService.buscarRmasCreasFechados(unidade, loginBean.getTenantId());
